...
//         const totalUsers = new Set(data.orders.map(order => order.user)).size;
...
//   if (loading) return <div className="pt-40 text-center font-prata">Loading...</div>;
...
//             className="border border-gray-200 px-4 py-3 text-base sm:text-[10px] font-bold tracking-widest uppercase outline-none focus:border-indigo-600 text-gray-900 bg-white h-11 w-full lg:w-56"
//           />
//           <button className="bg-indigo-600 text-white px-6 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-all h-11">
//             Add New
//           </button>
//         </div>
//       </div>
...
//         <div className="border border-gray-100 p-5 bg-white shadow-sm">
...
//         </div>
//         <div className="border border-gray-100 p-5 bg-white shadow-sm">
//           <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Users</p>
//           <h3 className="text-xl md:text-2xl font-bold font-prata text-gray-900">{stats.totalUsers.toLocaleString()}</h3>
//         </div>
//       </div>
...
//           <h3 className="text-[9px] font-bold uppercase tracking-[0.3em] text-gray-400">Recent Activity</h3>
//           <span className="text-[9px] font-mono text-gray-400">Count: {filteredOrders.length}</span>
//         </div>
...
//                 <th className="p-4">Manifest Items</th>
...
//                   <td className="p-4 font-mono text-gray-400 pt-5">{order._id}</td>
//                   <td className="p-4 font-bold text-gray-900 pt-5">{order.user?.username || "Unknown"}</td>
                  
//                   {/* Organized Items Column */}
//                   <td className="p-4 space-y-3">
//                     {order.items?.map((item, idx) => (
//                       <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3 text-xs">
//                         <span className="font-medium text-gray-800">{item.product?.name || "Unknown Product"}</span>
//                         <div className="flex items-center gap-2">
//                           {/* Size Tag */}
//                           {item.size && (
//                             <span className="bg-gray-100 text-gray-800 text-[9px] font-black px-1.5 py-0.5 border border-gray-200">
//                               {item.size}
//                             </span>
//                           )}
...
//                   <td className="p-4 font-bold text-indigo-600 pt-5">₦{order.totalAmount?.toFixed(2) || "0.00"}</td>
//                   <td className="p-4 text-right pt-5">
//                     <span className={`inline-flex items-center px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest ${order.status === "Delivered" ? "bg-green-50 text-green-700" : order.status === "Shipped" ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-700"}`}>
//                       {order.status}
//                     </span>
//                   </td>
//                 </tr>
...
//                 <span className="font-mono text-[9px] text-gray-400">{order._id}</span>
//                 <span className={`inline-flex items-center px-2 py-0.5 text-[8px] font-bold uppercase tracking-widest ${order.status === "Delivered" ? "bg-green-50 text-green-700" : order.status === "Shipped" ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-700"}`}>
//                   {order.status}
//                 </span>
//               </div>
              
//               <div>
//                 <h4 className="font-bold text-gray-900 text-xs mb-2">{order.user?.username || "Unknown"}</h4>
                
//                 {/* Mobile Organized Items List */}
//                 <div className="space-y-2.5 bg-gray-50/50 p-2.5 border border-gray-100">
//                   {order.items?.map((item, idx) => (
//                     <div key={idx} className="text-[11px] leading-tight">
//                       <div className="font-bold text-gray-800">{item.product?.name || "Unknown Product"}</div>
//                       <div className="flex items-center gap-2 mt-1">
//                         {item.size && (
//                           <span className="bg-white text-gray-700 text-[8px] font-black px-1.5 py-0.2 border border-gray-200">
//                             {item.size}
//                           </span>
//                         )}
//                         {item.color && (
...
//                 <span className="font-bold text-indigo-600">₦{order.totalAmount?.toFixed(2) || "0.00"}</span>
//               </div>
//             </div>
//           ))}
//         </div>
    ...
    <div className="w-full max-w-5xl mx-auto space-y-10">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end pb-4 gap-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input 
            ...
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="border border-gray-200 px-4 py-3 text-base sm:text-[10px] font-bold tracking-widest  outline-none focus:border-indigo-600 text-gray-900 bg-white h-11 w-full lg:w-56 rounded-none transition-all"
          />
          <button className="bg-indigo-600 text-white px-6 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-all h-11 active:scale-[0.99]">
            Add New
          </button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center border-b border-gray-100 pb-2.5">
          <h3 className="text-[9px] font-bold uppercase tracking-[0.3em] text-gray-400">Recent Activity</h3>
          <span className="text-[10px] font-mono text-gray-500 bg-gray-100 px-2 py-0.5 font-bold">Count: {filteredOrders.length}</span>
        </div>
                ...
                filteredOrders.toReversed().slice(0, 5).map((order) => {
                  ...
                  return (
                    <tr key={order._id} className="hover:bg-gray-50/70 transition-colors align-top group">
                      ...
                      <td className="p-4 space-y-2">
                        {order.items?.map((item, idx) => (
                          <div key={idx} className="bg-gray-50/60 p-2 border border-gray-100 flex items-center justify-between gap-4 max-w-md">
                            <div>
                              <div className="font-bold text-gray-900 text-xs">{item.product?.name || "Unknown Item"}</div>
                              <div className="flex items-center gap-3 text-[10px] mt-0.5 text-gray-500">
                                <span className="font-bold text-gray-700">Size: {item.size || "OS"}</span>
                                <span>•</span>
                                <ColorIndicator colorName={item.color} />
                              </div>
                            </div>
                            <span className="bg-white border border-gray-200 px-2 py-0.5 font-mono font-bold text-[10px] text-gray-600">
                              qty: {item.quantity || 1}
                            </span>
                          </div>
                        ))}
                      ...
                      <td className="p-4 font-bold text-gray-900 pt-5 text-sm">₦{order.totalAmount?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || "0.00"}</td>
                      <td className="p-4 text-right pt-5">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest border ${styles.badge}`}>
          ...
          {filteredOrders.length === 0 ? (
            ...
            filteredOrders.map((order) => {
              ...
              return (
                <div key={order._id} className="border border-gray-100 p-4 bg-white shadow-sm space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="font-mono text-[10px] text-gray-400 font-bold">{order._id}</span>
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-[8px] font-bold uppercase tracking-widest border ${styles.badge}`}>
                  ...
                  <div>
                    <span className="text-[8px] uppercase tracking-wider text-gray-400 block font-bold mb-0.5">Customer account</span>
                    ...
                    <div className="mt-3 space-y-2">
                      {order.items?.map((item, idx) => (
                        <div key={idx} className="bg-gray-50 p-2.5 border border-gray-100 flex justify-between items-start gap-2">
                          <div>
                            <div className="font-bold text-gray-800 text-xs leading-tight">{item.product?.name || "Unknown Product"}</div>
                            <div className="flex items-center gap-2 mt-1.5 text-[10px]">
                              <span className="bg-white px-1.5 py-0.5 border border-gray-200 font-black text-gray-700 text-[8px]">
                                {item.size || "OS"}
                              </span>
                              <ColorIndicator colorName={item.color} />
                            </div>
                  ...
                  <div className="pt-3 border-t border-gray-100 flex justify-between items-center">
                    <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold">Total Statement Amount</span>
                    <span className="font-bold text-indigo-600 text-sm">₦{order.totalAmount?.toLocaleString(undefined, { minimumFractionDigits: 2 }) || "0.00"}</span>
                  </div>
                </div>
              );
            })
