...
//     console.log(token);
//     try {
//       const response = await apiFetch("http://localhost:4001/api/v1/order/my-orders", {
//         headers: {
...
//   if (loading) return <div className="pt-40 text-center font-prata italic">Loading your wardrobe...</div>;
...
//           <p className="font-prata italic text-gray-400 mb-6">You haven&apos;t placed any orders yet.</p>
//           <button onClick={()=> router.push("/shop")} className="bg-black text-white px-10 py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-600 transition-all">
//             Start Shopping
//           </button>
//         </div>
//       ) : (
...
//           {orders?.map((order) => (
//             <div key={order._id} className="border border-gray-100 p-6 md:p-8 hover:shadow-sm transition-shadow bg-white">
//               {/* Order Header */}
...
//                     <p className="text-xs font-bold text-indigo-600">₦{order.totalAmount.toLocaleString()}</p>
//                   </div>
//                 </div>
//                 <div className="flex items-center gap-3">
//                     <span className={`w-2 h-2 rounded-full ${order.status === 'delivered' ? 'bg-green-500' : order.status === 'shipped' ? 'bg-blue-500' : order.status === 'cancelled' ? 'bg-red-500' : 'bg-orange-400'}`}></span>
//                     <p className="text-[10px] font-bold uppercase tracking-widest text-gray-700">{order.status}</p>
//                 </div>
//               </div>
...
//                       </div>
//                       <div>
//                         <h4 className="text-xs font-bold uppercase tracking-wide text-gray-900">{item.product.name}</h4>
//                         <p className="text-[10px] text-gray-400 mt-1">Size: {item.size} | Qty: {item.quantity} </p>
//                          {item.color ? <p className="text-[10px] text-gray-400 mt-1">Color: {item.color}</p> : null }
//                       </div>
//                     </div>
              ...
              <div className="flex flex-col md:flex-row justify-between mb-8 pb-6 border-b border-gray-50 gap-4">
                <div className="flex gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-900 uppercase">{order.paymentStatus || "Pending"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                    <span className={`w-2 h-2 rounded-full ${order.status === 'delivered' ? 'bg-green-500' : order.status === 'shipped' ? 'bg-blue-500' : order.status === 'cancelled' ? 'bg-red-500' : 'bg-orange-400'}`}></span>
