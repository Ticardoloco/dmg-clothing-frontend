...
const VintagePage = () => {
  ...
  const loadProduct = async () => {
    ...
    try {
      ...
      const filterProducts = vintageData.filter(
        (item) => item.subCategory === "Vintage",
      );
  ...
  const sortProducts = () => {
    let fCopy = [...vintageProducts];

    switch (sortType) {
      case "low-high":
        setVintageProducts(fCopy.sort((a, b) => a.price - b.price));
        break;
      case "high-low":
        setVintageProducts(fCopy.sort((a, b) => b.price - a.price));
        break;
      default:
        vintageProducts;
        break;
  ...
  return (
    <div className="pt-24 pb-16 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex gap-2 text-xs uppercase tracking-widest text-gray-400 mb-10">
        ...
        <span>/</span>
        <span className="text-gray-900 font-bold">Vintage</span>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6 md:mb-12 border-b border-gray-100 pb-5 md:pb-10">
        <div className="max-w-2xl">
          <h1 className="text-4xl md:text-6xl font-bold font-prata text-gray-900 uppercase leading-tight">
            The{" "}
            <span className="text-indigo-600 italic font-light">Vintage</span>{" "}
            Series
          </h1>
          <p className="text-gray-500 mt-4 text-base md:text-lg leading-relaxed">
            A curated selection of prints and patterns that transcend time. From
            70s silk collars to 90s oversized silhouettes, our Vintage
            collection brings character and soul back to your wardrobe.
          </p>
        </div>

        <div className="flex items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
            {vintageProducts.length} Products
          </p>
          <select
          ...
          >
            <option value="relevant">Sort: Relavant</option>
      ...
      {!isLoading && vintageProducts.length === 0 && (
        <div className="py-32 text-center">
          <p className="text-gray-400 font-prata italic text-lg">
            Exploring new fabrics. Check back soon.
          </p>
        </div>
      )}
    </div>
