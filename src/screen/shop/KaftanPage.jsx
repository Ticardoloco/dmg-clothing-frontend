...
const KaftanPage = () => {
  ...
  const loadProduct = async () => {
    ...
    try {
      ...
      const filterProducts = kaftanData.filter(
        (item) => item.subCategory === "Kaftan",
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6 md:mb-12 border-b border-gray-100 pb-5 md:pb-10">
        <div className="max-w-2xl">
          <h1 className="text-4xl md:text-6xl font-bold font-prata text-gray-900 uppercase leading-tight">
            The{" "}
            <span className="text-indigo-600 italic font-light">Kaftan</span>{" "}
            Series
          </h1>
          <p className="text-gray-500 mt-4 text-base md:text-lg leading-relaxed">
            Sophisticated, comfortable, and timeless. Our Kaftan collection is
            tailored for the modern man who values effortless style. From
            breathable linens to rich silks, find the perfect piece for any
            occasion.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
            {kaftanProducts.length} Products
          </p>
          <select
      ...
      {!isLoading && kaftanProducts.length === 0 && (
        <div className="py-32 text-center">
          <p className="text-gray-400 font-prata italic text-lg">
            Our Kaftan inventory is currently being refreshed.
          </p>
          <Link
            href="/shop"
            className="mt-4 inline-block text-xs font-bold uppercase tracking-widest border-b border-black pb-1"
          >
            Back to Shop
          </Link>
        </div>
