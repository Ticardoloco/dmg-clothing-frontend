...
const JacketPage = () => {
  ...
  const loadProduct = async () => {
    ...
    try {
      ...
      const filterProducts = jacketData.filter(
        (item) => item.subCategory === "Jacket",
      );
  ...
  const sortProducts = () => {
    let fCopy = jacketProducts.slice();

    switch (sortType) {
      case "low-high":
        setJackProducts(fCopy.sort((a, b) => a.price - b.price));
        break;
      case "high-low":
        setJackProducts(fCopy.sort((a, b) => b.price - a.price));
        break;
      default:
        jacketProducts;
        break;
  ...
  return (
    <div className="pt-24 pb-16 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex gap-2 text-xs uppercase tracking-widest text-gray-400 mb-10">
        ...
        <span>/</span>
        <span className="text-gray-900 font-bold">Jackets</span>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6 md:mb-12 border-b border-gray-100 pb-5 md:pb-10">
        <div className="max-w-2xl">
          <h1 className="text-4xl md:text-6xl font-bold font-prata text-gray-900 uppercase leading-tight">
            The{" "}
            <span className="text-indigo-600 italic font-light">Jacket</span>{" "}
            Series
          </h1>
          <p className="text-gray-500 mt-4 text-base md:text-lg leading-relaxed">
            Structured for the modern silhouette. From ceremonial vests that
            honor tradition to utility outerwear for the urban landscape, our
            jackets are designed to be the defining layer of your attire.
          </p>
        </div>

        <div className="flex items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
            {jacketProducts.length} Outerwear Pieces
          </p>
          <select
      ...
      {!isLoading && jacketProducts.length === 0 && (
        <div className="py-32 text-center">
          <p className="text-gray-400 font-prata italic text-lg">
            Tailoring in progress. New outer layers arriving soon.
          </p>
        </div>
      )}
    </div>
