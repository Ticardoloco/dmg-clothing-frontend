...
const JalabiyaPage = () => {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      const filterJalabiya = jalabiyaData.filter(
        (item) => item.subCategory === "Jalabiya",
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6 md:mb-12 border-b border-gray-100 pb-5 md:pb-10">
        <div className="max-w-2xl">
          <h1 className="text-4xl md:text-6xl font-bold font-prata text-gray-900 uppercase leading-tight">
            The{" "}
            <span className="text-indigo-600 italic font-light wh">
              Jalabiya
            </span>{" "}
            Series
          </h1>
          <p className="text-gray-500 mt-4 text-base md:text-lg leading-relaxed">
            Traditional silhouettes reimagined for the modern world. Our
            Jalabiyas offer unparalleled comfort without compromising on
            presence. Meticulously tailored for a clean, flowing drape.
          </p>
        </div>

        <div className="flex items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
            {jalabiyaProducts.length} Products
          </p>
          <select
