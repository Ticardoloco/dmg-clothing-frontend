...
const AgbadaPage = () => {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      const filterAgada = AgbadaData.filter(
        (item) => item.subCategory === "Agbada",
      );
    ...
    <div className="pt-24 pb-16 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6 md:mb-12 border-b border-gray-100 pb-5 md:pb-10">
        <div className="max-w-2xl">
          <h1 className="text-4xl md:text-6xl font-bold font-prata text-gray-900 uppercase leading-tight">
            The{" "}
            <span className="text-indigo-600 italic font-light">Agbada</span>{" "}
            Series
          </h1>
          <p className="text-gray-500 mt-4 text-base md:text-lg leading-relaxed">
            The pinnacle of African prestige. Our Agbada collection combines
            voluminous traditional silhouettes with modern embroidery
            techniques, designed for the man who commands respect in every room.
          </p>
        </div>

        <div className="flex items-center gap-4">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-400">
            {agbadaProducts?.length} Products
          </p>
          <select
      ...
      {!isLoading && agbadaProducts?.length === 0 && (
        <div className="py-20 text-center">
          <p className="text-gray-400 font-prata italic">
            No Agbada styles found in this collection.
          </p>
        </div>
      )}
    </div>
